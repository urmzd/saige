...
func TestNullableToolSchemaOnWire(t *testing.T) {
	...
	raw, err := json.Marshal(toOllamaTools(defs))
	if err != nil {
		t.Fatal(err)
	}
	var wire []any
...
func TestNullableStructuredOutputSchema(t *testing.T) {
	schema := parameterSchemaToMap(types.ParameterSchema{Type: "object", Properties: map[string]types.PropertyDef{
		"owner": {Type: "string", Nullable: true},
	}})
	p := schema["properties"].(map[string]any)["owner"].(map[string]any)
	if !reflect.DeepEqual(p["type"], []string{"string", "null"}) {
		t.Fatalf("format schema lost null: %v", p)
	}
}
